...
- **Inbound:** PHL → MAD (X & E; nonstop exists on the AA/Iberia joint venture) and SIN → MAD (parents; one-stop — compare hubs on price/timing).
- **Outbound, X & E:** BIO → MAD → PHL on Iberia. May be ticketed as separate tickets for pricing reasons, but planned as a connecting itinerary — ⚠️ if actually on separate tickets, leave a generous MAD connection (3 h+) and carry-on-friendly bags, since a misconnect is on us, not the airline.
- **Outbound, parents:** BIO → MAD on Iberia (frequent shuttle, ~1 h), **overnight at a Madrid airport hotel**, then MAD → SIN the next day. Booking the airport night removes all connection stress and gives them a calm final evening.
...
## Booking checklist (details in advance-bookings.md)
